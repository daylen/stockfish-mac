...
public:
  ...
  bool is_mate();
  bool is_draw() const;
  DrawReason is_immediate_draw() const;
  ...
  int moves_from(Square s, Move mlist[]) const;
  int moves_to(Square s, Move mlist[]) const;
  int destination_squares_from(Square s, Square sqlist[]) const;
  int all_legal_moves(Move mlist[]) const;

  // Position consistency check, for debugging
  bool is_ok(bool slow = false) const;
