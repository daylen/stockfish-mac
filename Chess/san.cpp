...
const std::string move_to_san(Position &pos, Move m, bool figurine) {
  std::string str;
  ...
  if(m == MOVE_NONE) {
    ...
    if(type_of_piece(pc) == PAWN) {
      ...
      if (figurine)
         str += figurines[type_of_piece(pc)];
      else
    ...
    if(move_promotion(m)) {
      str += "=";
      if (figurine)
        str += figurines[move_promotion(m)];
      else
...
/// move_to_html() is similar to move_to_san(), except that it uses a
/// non-breaking hyphen for castling moves, and uses optional figurine
/// notation.

const std::string move_to_html(Position &pos, Move m, bool figurine) {
   if (move_is_long_castle(m))
      return "O&#8209;O&#8209;O";
   else if (move_is_short_castle(m))
      return "O&#8209;O";
   else
      return move_to_san(pos, m, figurine);
}
...
Move move_from_san(Position &pos, const std::string &movestr) {
  ...
  // Castling moves
  if(movestr == "O-O-O" || movestr == "O-O-O+" || movestr == "O-O-O#") {
    Move m;
  ...
  else if(movestr == "O-O" || movestr == "O-O+" || movestr == "O-O#") {
    Move m;
    while((m = mp.get_next_move()) != MOVE_NONE)
...
const std::string line_to_san(const Position &pos, Move line[], int startColumn,
                              bool breakLines, int moveNumbers) {
   Position p = Position(pos);
   UndoInfo u;
   std::stringstream s, ns;
   std::string moveStr;
   int length, maxLength;
   int add = p.side_to_move() == WHITE ? 0 : 1;
...
const std::string line_to_html(const Position &pos, Move line[], int currentPly, bool figurine) {
   Position p = Position(pos);
   UndoInfo u;
   std::stringstream s, ns;
   ...
   for(int i = 0; line[i] != MOVE_NONE; i++) {
      ...
      moveStr.str("");
      moveStr << "<a name=\"ply" << i << "\">";
      moveStr << "<span class=\"move\" id=\"ply" << i << "\">";
      if (i == currentPly - 1)
         moveStr << "<strong>" + move_to_html(p, line[i], figurine) + "</strong>";
      else
         moveStr << move_to_html(p, line[i], figurine);
      moveStr << "</span>";
      moveStr << "</a>";
      
      length += moveStr.str().length() + 1;
      if(breakLines && length > maxLength) {
         s << "\n";
         for(int j = 0; j < startColumn; j++)
            s << " ";
         length = (int)moveStr.str().length() + 1;
      }
      s << ns.str() << moveStr.str() << " ";
