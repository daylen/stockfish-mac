...
- (NSAttributedString *)longestFlatLineFrom:(SFMNode *)node position:(SFMPosition *)position
{
    ...
    if(node.comment != nil){
        NSString *commentWithBraces = [NSString stringWithFormat:@"{%@} ", node.comment];
        [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:commentWithBraces attributes:@{NSLinkAttributeName: self.commentIdentifier}]];
    }
